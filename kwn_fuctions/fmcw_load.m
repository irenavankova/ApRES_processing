...
% Check args
if nargin == 0
    [filename, path] = uigetfile(['*.dat;*.DAT;*.000;*.mat'],'Choose radar file to plot','multiselect','on');
...
if FileFormat == 6
    vdat.vif = zeros(vdat.ChirpsInBurst,SamplesPerChirp * 2); % preallocate array
else
...
for chirp = 1:vdat.ChirpsInBurst
    %size(vdat.v(vdat.Startind(chirp):vdat.Endind(chirp)))
    %size(vdat.vif(chirp,:))
    %--IV modification HEL1 2015 and 2016 make problems otherwise
    if length((vdat.v(vdat.Startind(chirp):vdat.Endind(chirp)))) > length(vdat.vif(chirp,:))
        vdat.vif(chirp,:) = vdat.v(vdat.Startind(chirp):vdat.Startind(chirp)+length(vdat.vif(chirp,:))-1);
    else
