function [out] = ct_tseries_via_xcorr_sliding_window(cfg,site)
%--------------------------------------------------------------
...
% The bursts are spaced by a window
% Time is the mean time between the burst intervals
%--------------------------------------------------------------
...
swin = cfg.xcor_sliding_window; % size of sliding window
NN = size(site.spec_cor,1)-swin;
...
for j = i1:NN
    ...
    g.specCor = site.spec_cor(j+swin,:);
    ...
    % Do fine allingment
    [~,dh,~] = ct_align_fine(cfg,f,g,AC); % alternatively output [AF,dh,dhe]
    
    out.dh(j,:) = dh;
    out.time(j) = site.time(j) + (site.time(j+swin)-site.time(j))/2;
